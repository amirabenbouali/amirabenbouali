...
describe('App routes', () => {
  it('renders the homepage dream opening semantics', async () => {
    renderWithRouter(routes);

    expect((await screen.findAllByRole('heading', { name: /Amira Benbouali lucid portfolio foundation/i })).length).toBeGreaterThan(0);
  ...
  it('allows the cinematic introduction to be skipped', async () => {
    const scrollTo = vi.spyOn(window, 'scrollTo').mockImplementation(() => undefined);
    renderWithRouter(routes);

    fireEvent.click(await screen.findByRole('button', { name: 'Skip experience' }));

    expect(scrollTo).toHaveBeenCalled();
    scrollTo.mockRestore();
  });

  it('renders a project case study shell from project data', async () => {
    renderWithRouter(routes, '/work/atria');

    expect(await screen.findByRole('heading', { name: 'Atria' })).toBeInTheDocument();
    expect(screen.getByText(/calm planning environment/i)).toBeInTheDocument();
  });

  it('renders the writing journal index and an article page', async () => {
  ...
  it('renders the about page as a reflective essay', async () => {
    renderWithRouter(routes, '/about');

    expect(await screen.findByRole('heading', { name: /making thought usable/i })).toBeInTheDocument();
